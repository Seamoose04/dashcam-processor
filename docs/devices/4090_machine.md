...
* Extracting low-resolution frames or thumbnails at full framerate but low resolution
* Motion filtering to discard empty/useless frames
* Coral-based plate region proposals (previously handled by Jetson)
* Basic heuristics for lighting, blur, and distance estimation
...
## 3.1 Preprocessing & Frame Extraction
* Runs low-resolution frame extraction at full framerate but reduced resolution
* Performs motion filtering to discard empty or useless frames
* Generates coarse plate region proposals based on Coral TPU processing
* Applies basic heuristics (lighting, blur, distance estimation)
* Produces small preprocessed artifacts for downstream use

## 3.2 YOLO Full-Resolution Detection
* Runs high-accuracy YOLO model on selected frames
* Uses extracted region proposals to limit search space
* Outputs bounding boxes for:
  * License plates
  * Vehicles
  * Optional: headlights, brake lights, pedestrians, etc.

## 3.3 Plate Crop Extraction
