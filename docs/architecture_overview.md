...
* **Jetson+Coral:** Light preprocessing + filtering
...
Jetson+Coral     | Preproc JSON + thumbs (intermediate)  | Indoor NAS (/videos/preproc)
...
* Pulls preprocessing tasks
* Reads raw videos from the Indoor NAS
* Performs:

  * Frame extraction (quick, low-res)
  * Motion filtering
  * Coral-based plate region proposals
  * Metadata collection
* Stores temporary results locally
* Publishes a remote heavy-processing task on completion
...
4. **Jetson pulls preprocessing task**, extracts useful info
5. Jetson publishes remote heavy-processing task
6. **4090 pulls heavy-processing task**, runs YOLO + OCR + GPS alignment
7. 4090 publishes archival task
8. **Server finalizes**, sends archival data to Shed NAS
9. **Shed NAS stores** the archived outputs
10. **WebUI displays** detections, crops, GPS route, timeline
...
This makes the workflow extremely durable.
...
* **Clear separation of concerns:** Ingestion → Preprocessing → Heavy → Archival → UI
...
This architecture provides a robust, interruption-safe, distributed workflow for processing dashcam videos end-to-end. Every device participates using a unified task model, and the system flows cleanly through ingestion, preprocessing, heavy GPU work, and final archival storage.
